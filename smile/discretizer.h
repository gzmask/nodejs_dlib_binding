...
public:
	...
	int Discretize(MethodType method, int intervals, std::vector<double> &edges) { return DoDiscretize(method, intervals, &edges, NULL); }
	int Discretize(MethodType method, int intervals, std::vector<int> &discretized) { return DoDiscretize(method, intervals, NULL, &discretized); }
