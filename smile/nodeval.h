...
public:
  ...
  virtual int GetSoftEvidence(std::vector<double> &evidence) const { return DSL_WRONG_NODE_TYPE; }
  virtual int SetSoftEvidence(const std::vector<double> &evidence) { return DSL_WRONG_NODE_TYPE; }
  ...
  int IsSoftEvidence() const { return IsFlagSet(DSL_VALUE_SOFT_EVIDENCE); }
